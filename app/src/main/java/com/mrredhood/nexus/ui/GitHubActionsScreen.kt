...
fun GitHubActionsScreen(project: NexusProject, onBack: () -> Unit) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Actions") }, navigationIcon = { TextButton(onClick = onBack) { Text("‹") } }) }) { padding ->
        LazyColumn(modifier = Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            items(workflows, key = { it.id }) { workflow ->
                Card(modifier = Modifier.fillMaxWidth()) { Column(modifier = Modifier.padding(14.dp)) { Text(workflow.name, style = MaterialTheme.typography.titleMedium); Text(workflow.path); Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
    ...
    selectedRun?.let { run ->
        AlertDialog(onDismissRequest = { selectedRun = null }, title = { Text(run.name) }, text = { LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            items(jobs, key = { it.id }) { job -> Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Column(modifier = Modifier.weight(1f)) { Text(job.name); Text("${job.status} · ${job.conclusion ?: "running"}") }; TextButton(onClick = { val accessToken = token() ?: return@TextButton; scope.launch { loading = true; runCatching { logs = service.workflowJobLogs(repository, job.id, accessToken) }.onFailure { error = it.message }; loading = false } }) { Text("Logs") } } }
            item { Text("Artifacts", style = MaterialTheme.typography.titleMedium) }
            items(artifacts, key = { it.id }) { artifact -> Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Column(modifier = Modifier.weight(1f)) { Text(artifact.name); Text("${artifact.sizeBytes / 1024} KB · ${if (artifact.expired) "expired" else "available"}") }; TextButton(enabled = !artifact.expired && !artifact.downloadUrl.isNullOrBlank(), onClick = { context.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(artifact.downloadUrl))) }) { Text("Open") } } }
    ...
    logs?.let { content -> AlertDialog(onDismissRequest = { logs = null }, title = { Text("Job logs") }, text = { LazyColumn { item { Text(content, style = MaterialTheme.typography.bodySmall) } } }, confirmButton = { TextButton(onClick = { logs = null }) { Text("Close") } }) }
}
