...
fun AdvancedSettingsSection() {
    ...
    var apiKey by remember { mutableStateOf("") }; var testRunning by remember { mutableStateOf(false) }; var testMessage by remember { mutableStateOf<String?>(null) }; var testSuccess by remember { mutableStateOf(false) }
    ...
    LaunchedEffect(settings.provider) { apiKey = ""; testMessage = null; endpoint = settings.endpoint; filter = "all"; vm.loadModels(settings.provider) }
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        SettingsSection("AI Providers & Models") {
            ...
            Text("Keys are encrypted with Android Keystore and never written to DataStore, project files, chat history, or Git. Each provider keeps its own key and live model list.", style = MaterialTheme.typography.bodySmall)
        }
