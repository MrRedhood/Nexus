...
fun AdvancedSettingsSection() {
    ...
    LaunchedEffect(settings.provider) {
        apiKey = ""; testMessage = null; endpoint = settings.endpoint; filter = "all"; vm.loadModels(settings.provider)
    }

    val filteredModels = models.filter { when (filter) {
    ...
    Column(Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        SettingsSection("AI Providers & Models") {
            ...
            Text("API keys are encrypted with Android Keystore and are never written to DataStore, project files, chat history, or Git. Models are loaded live from the selected provider.", style = MaterialTheme.typography.bodySmall)
        }
