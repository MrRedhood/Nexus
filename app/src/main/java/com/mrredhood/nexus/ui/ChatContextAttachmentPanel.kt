...
@Composable
fun ChatContextAttachmentPanel(workspace: Workspace, fileSystem: WorkspaceFileSystem, context: ChatContext, onContextChanged: (ChatContext) -> Unit) {
    val baseCurrentFile = remember(workspace.id) { mutableStateOf(context.currentFile) }
    val baseSelection = remember(workspace.id) { mutableStateOf(context.selection) }
    val baseGitDiff = remember(workspace.id) { mutableStateOf(context.gitDiff) }
    val baseTerminal = remember(workspace.id) { mutableStateOf(context.terminalOutput) }
    val baseWorkspace = remember(workspace.id) { mutableStateOf(context.workspaceSummary) }
    var pickerOpen by remember { mutableStateOf(false) }
    var expanded by remember { mutableStateOf(false) }

    fun apply(current: Boolean = context.includeCurrentFile, selection: Boolean = context.includeSelection, git: Boolean = context.includeGitDiff, terminal: Boolean = context.includeTerminalOutput, workspaceSummary: Boolean = context.includeWorkspaceSummary, attachments: List<ChatAttachment> = context.attachedFiles) {
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Column(Modifier.weight(1f)) { Text("Chat context", style = MaterialTheme.typography.titleSmall); Text("$sourceCount sources selected", style = MaterialTheme.typography.labelSmall) }
            ...
            if (context.attachedFiles.isNotEmpty()) {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                    context.attachedFiles.take(4).forEach { attachment -> FilterChip(selected = true, onClick = { apply(attachments = context.attachedFiles.filterNot { it.path == attachment.path }) }, label = { Text(attachment.path.substringAfterLast('/')) }) }
    ...
    if (pickerOpen) WorkspaceFilePicker(workspace, fileSystem, context.attachedFiles.map { it.path }.toSet(), { pickerOpen = false }) { attachment -> apply(attachments = context.attachedFiles.filterNot { it.path == attachment.path } + attachment) }
}

@Composable
private fun ContextToggle(label: String, selected: Boolean, onClick: () -> Unit) { FilterChip(selected = selected, onClick = onClick, label = { Text(label) }) }

@Composable
private fun WorkspaceFilePicker(workspace: Workspace, fileSystem: WorkspaceFileSystem, selectedPaths: Set<String>, onDismiss: () -> Unit, onAttach: (ChatAttachment) -> Unit) {
    ...
    LaunchedEffect(workspace.id, directory) {
        ...
        runCatching { fileSystem.list(workspace, directory) }.onSuccess { entries = it }.onFailure { error = it.message ?: "Unable to list workspace files." }
        loading = false
    }

    AlertDialog(onDismissRequest = onDismiss, title = { Text("Attach workspace file") }, text = {
        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            LazyColumn(Modifier.heightIn(max = 420.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                items(entries) { entry ->
                    if (entry.type == EntryType.DIRECTORY) OutlinedButton(onClick = { directory = entry.relativePath }, modifier = Modifier.fillMaxWidth(), enabled = !loading) { Text("Folder: ${entry.name}") }
                    else {
                        val attached = entry.relativePath in selectedPaths
                        OutlinedButton(onClick = {
                            if (!attached && !loading) { loading = true; scope.launch { runCatching { fileSystem.read(workspace, entry.relativePath) }.onSuccess { file -> onAttach(ChatAttachment(file.relativePath, file.content, file.sizeBytes)) }.onFailure { error = it.message ?: "Unable to read file." }; loading = false } }
                        }, modifier = Modifier.fillMaxWidth(), enabled = !loading) {
                            Column(Modifier.weight(1f)) { Text(entry.name); Text("${entry.sizeBytes} bytes", style = MaterialTheme.typography.labelSmall) }
