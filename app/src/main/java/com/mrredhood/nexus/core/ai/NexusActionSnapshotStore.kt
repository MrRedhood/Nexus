...
class NexusActionSnapshotStore(private val fileSystem: WorkspaceFileSystem) {
    ...
    suspend fun capture(workspace: Workspace, action: NexusAction): String? {
        ...
        val id = action.id
        snapshots[id] = Snapshot(roots, entries.values.toList())
    ...
    suspend fun rollback(workspace: Workspace, snapshotId: String): Boolean {
        ...
        snapshot.roots.sortedByDescending { it.count { c -> c == '/' } }.forEach { root -> if (fileSystem.exists(workspace, root)) deleteTree(workspace, root) }
        snapshot.entries.filter { it.isDirectory }.sortedBy { it.path.count { c -> c == '/' } }.forEach { entry -> if (!fileSystem.exists(workspace, entry.path)) runCatching { fileSystem.createDirectory(workspace, entry.path) } }
        snapshot.entries.filter { !it.isDirectory }.sortedBy { it.path }.forEach { entry -> fileSystem.write(workspace, entry.path, entry.content.orEmpty(), entry.mimeType ?: "text/plain") }
    ...
    private suspend fun capturePath(workspace: Workspace, path: String, output: MutableMap<String, SnapshotEntry>) {
        ...
        if (fileSystem.isDirectory(workspace, path)) {
            output[path] = SnapshotEntry(path, true, null, null)
            ...
            output[path] = SnapshotEntry(path, false, file.content, file.mimeType)
        }
    }

    private suspend fun deleteTree(workspace: Workspace, path: String) {
        if (fileSystem.isDirectory(workspace, path)) fileSystem.list(workspace, path).forEach { deleteTree(workspace, it.relativePath) }
    ...
    private fun affectedRoots(action: NexusAction): List<String> = buildList {
        ...
        if (action.type == "rename_file") {
            val path = action.path?.trim().orEmpty(); val name = action.newName?.trim().orEmpty()
            if (path.isNotBlank() && name.isNotBlank()) add(path.substringBeforeLast('/', "").let { if (it.isBlank()) name else "$it/$name" })
        }
    }.distinct()

    private data class Snapshot(val roots: List<String>, val entries: List<SnapshotEntry>)
    private data class SnapshotEntry(val path: String, val isDirectory: Boolean, val content: String?, val mimeType: String?)
