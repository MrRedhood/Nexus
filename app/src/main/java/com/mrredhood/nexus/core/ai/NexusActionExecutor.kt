...
class NexusActionExecutor(private val fileSystem: WorkspaceFileSystem) {
    ...
    suspend fun execute(workspace: Workspace, action: NexusAction): ActionExecutionResult {
        val result = runCatching {
            when (action.type) {
                ...
                "create_file" -> { val path = requirePath(action); val content = action.content.orEmpty(); val file = fileSystem.write(workspace, path, content, action.mimeType ?: "text/plain"); ActionExecutionResult(true, "Created $path (+${physicalLineCount(content)} lines)", file.content) }
        ...
        NexusActionExecutionRegistry.record(NexusActionExecutionSummary(action.id, action.type, action.path, result.success, counts.first, counts.second, result.message))
        return result
    }

    private fun physicalLineCount(content: String): Int {
    ...
    private fun applyUnifiedPatch(original: String, patch: String): String {
        ...
        for (hunkIndex in hunks.indices) {
            ...
            originalLines.subList(oldStart, sourceIndex).clear(); originalLines.addAll(oldStart, replacement)
            offset += replacement.size - consumed; cursor = nextHeader
        }
