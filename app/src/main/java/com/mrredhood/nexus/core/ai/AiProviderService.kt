...
class AiProviderService(context: Context) {
    ...
    suspend fun stream(request: AiRequest, onDelta: suspend (String) -> Unit): ProviderResult = withContext(Dispatchers.IO) {
        execute(request, true) { delta -> emitSmooth(delta, onDelta) }
    }
    ...
    private suspend fun execute(request: AiRequest, streaming: Boolean, onDelta: suspend (String) -> Unit): ProviderResult {
        val config = settings.settings.first(); val provider = config.provider.trim(); val key = keys.get(provider) ?: return failure("missing_api_key", "No API key configured for $provider.")
        val primary = resolveModel(provider, request.model, config.model); if (primary.isBlank()) return failure("missing_model", "No model configured for $provider.")
        val candidates = buildList { add(primary); request.fallbackModels.map { it.trim() }.filter { it.isNotBlank() && it != primary }.forEach(::add) }
        var totalAttempts = 0; var last: ProviderResult? = null
        for (model in candidates) {
            val result = runWithRetry(provider, key, model, request, config.endpoint, streaming, onDelta); totalAttempts += result.attempts
    ...
    private suspend fun runWithRetry(provider: String, key: String, model: String, request: AiRequest, endpoint: String, streaming: Boolean, onDelta: suspend (String) -> Unit): ProviderResult {
        val maxAttempts = if (streaming) 1 else 3; var last: ProviderResult? = null
        for (attempt in 1..maxAttempts) {
            val result = try {
                when { provider.equals("Gemini", true) -> gemini(key, model, request, streaming, onDelta); provider.equals("Anthropic", true) -> anthropic(key, model, request, endpoint, streaming, onDelta); else -> openAi(provider, key, model, request, endpoint, streaming, onDelta) }
            } catch (e: CancellationException) { throw e } catch (e: Throwable) { failure("request_failed", e.message ?: "AI request failed.") }
    ...
    private suspend fun emitSmooth(delta: String, onDelta: suspend (String) -> Unit) {
        if (delta.isEmpty()) return
        var index = 0
        while (index < delta.length) {
            val end = (index + 4).coerceAtMost(delta.length)
            onDelta(delta.substring(index, end))
            index = end
            if (index < delta.length) delay(16)
        }
    }
    ...
    private suspend fun gemini(key: String, model: String, request: AiRequest, streaming: Boolean, onDelta: suspend (String) -> Unit): ProviderResult {
        ...
        val body = JSONObject().apply { put("contents", JSONArray().also { a -> request.messages.filter { it.role != "system" }.forEach { m -> a.put(JSONObject().put("role", if (m.role == "assistant") "model" else "user").put("parts", JSONArray().put(JSONObject().put("text", m.content)))) } }); request.messages.firstOrNull { it.role == "system" }?.let { put("systemInstruction", JSONObject().put("parts", JSONArray().put(JSONObject().put("text", it.content)))) }; put("generationConfig", JSONObject().put("temperature", request.temperature).put("maxOutputTokens", request.maxOutputTokens)) }
    ...
    private suspend fun anthropic(key: String, model: String, request: AiRequest, configuredEndpoint: String, streaming: Boolean, onDelta: suspend (String) -> Unit): ProviderResult {
        val endpoint = configuredEndpoint.trim().ifBlank { "https://api.anthropic.com/v1/messages" }; val system = request.messages.firstOrNull { it.role == "system" }?.content
        val body = JSONObject().apply { put("model", model); put("max_tokens", request.maxOutputTokens); put("temperature", request.temperature); put("stream", streaming); system?.let { put("system", it) }; put("messages", JSONArray().also { a -> request.messages.filter { it.role != "system" }.forEach { m -> a.put(JSONObject().put("role", if (m.role == "assistant") "assistant" else "user").put("content", m.content)) } }) }
        return readResponse(endpoint, body.toString(), key, streaming, "Anthropic", model, onDelta) { json -> if (streaming) json.optJSONObject("delta")?.optString("text").orEmpty() else json.optJSONArray("content")?.optJSONObject(0)?.optString("text").orEmpty() }
    }
    ...
    private suspend fun openAi(provider: String, key: String, model: String, request: AiRequest, configuredEndpoint: String, streaming: Boolean, onDelta: suspend (String) -> Unit): ProviderResult {
        val endpoint = configuredEndpoint.trim().ifBlank { defaultEndpoint(provider) }; if (endpoint.isBlank()) return failure("missing_endpoint", "No endpoint configured for $provider.")
        val body = JSONObject().apply { put("model", model); put("messages", JSONArray().also { a -> request.messages.forEach { a.put(JSONObject().put("role", it.role).put("content", it.content)) } }); put("temperature", request.temperature); put("max_tokens", request.maxOutputTokens); put("stream", streaming); if (streaming) put("stream_options", JSONObject().put("include_usage", true)) }
    ...
    private suspend fun readResponse(endpoint: String, body: String, auth: String?, streaming: Boolean, provider: String, model: String, onDelta: suspend (String) -> Unit, extract: (JSONObject) -> String): ProviderResult {
        val connection = (URL(endpoint).openConnection() as HttpURLConnection).apply { requestMethod = "POST"; connectTimeout = 20_000; readTimeout = 120_000; doOutput = true; setRequestProperty("Content-Type", "application/json"); setRequestProperty("Accept", if (streaming) "text/event-stream" else "application/json"); if (provider.equals("Anthropic", true)) { setRequestProperty("x-api-key", auth ?: ""); setRequestProperty("anthropic-version", "2023-06-01") } else auth?.let { setRequestProperty("Authorization", it) } }
        ...
        try {
            connection.outputStream.use { it.write(body.toByteArray(Charsets.UTF_8)) }; val status = connection.responseCode
            if (status !in 200..299) { val errorBody = (connection.errorStream ?: connection.inputStream).bufferedReader().use { it.readText() }; val error = classifyHttpError(status, errorBody); return ProviderResult(false, error.message, error = error) }
            if (streaming) connection.inputStream.bufferedReader().useLines { lines -> lines.forEach { line -> if (!line.startsWith("data:")) return@forEach; val data = line.removePrefix("data:").trim(); if (data.isBlank() || data == "[DONE]") return@forEach; val json = runCatching { JSONObject(data) }.getOrNull() ?: return@forEach; val usage = extractUsage(json); if (usage != null) { inputTokens = usage.first ?: inputTokens; outputTokens = usage.second ?: outputTokens; totalTokens = usage.third ?: totalTokens }; val delta = runCatching { extract(json) }.getOrDefault(""); if (delta.isNotEmpty()) { text.append(delta); onDelta(delta) } } }
            else { val json = JSONObject(connection.inputStream.bufferedReader().use { it.readText() }); text.append(extract(json)); val usage = extractUsage(json); inputTokens = usage?.first; outputTokens = usage?.second; totalTokens = usage?.third }
            return ProviderResult(true, text.toString(), response = AiResponse(text.toString(), model, provider, inputTokens, outputTokens, totalTokens))
        } catch (e: CancellationException) { throw e }
        catch (e: java.net.SocketTimeoutException) { val error = AiError("timeout", "The $provider request timed out.", retryable = true); return ProviderResult(false, error.message, error = error) }
        catch (e: java.io.IOException) { val error = AiError("network_error", e.message ?: "Network error while contacting $provider.", retryable = true); return ProviderResult(false, error.message, error = error) }
        catch (e: Throwable) { val error = AiError("parse_error", e.message ?: "Invalid response from $provider."); return ProviderResult(false, error.message, error = error) }
