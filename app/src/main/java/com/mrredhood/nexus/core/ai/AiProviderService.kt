...
class AiProviderService(context: Context) {
    ...
    private suspend fun openAi(provider: String, key: String, model: String, request: AiRequest, configuredEndpoint: String, streaming: Boolean, onDelta: suspend (String) -> Unit): ProviderResult {
        val endpoint = configuredEndpoint.trim().ifBlank { defaultEndpoint(provider) }; require(endpoint.isNotBlank()) { "No endpoint configured for $provider." }
        val body = JSONObject().apply { put("model", model); put("messages", JSONArray().also { a -> request.messages.forEach { a.put(JSONObject().put("role", it.role).put("content", it.content)) } }); put("temperature", request.temperature); put("max_tokens", request.maxOutputTokens); put("stream", streaming) }
    ...
    private suspend fun readResponse(endpoint: String, body: String, auth: String?, streaming: Boolean, provider: String, model: String, onDelta: suspend (String) -> Unit, extract: (JSONObject) -> String): ProviderResult {
        val c = (URL(endpoint).openConnection() as HttpURLConnection).apply { requestMethod = "POST"; connectTimeout = 20_000; readTimeout = 120_000; doOutput = true; setRequestProperty("Content-Type", "application/json"); setRequestProperty("Accept", if (streaming) "text/event-stream" else "application/json"); auth?.let { setRequestProperty("Authorization", if (provider.equals("Anthropic", true)) "" else it) }; if (provider.equals("Anthropic", true)) { setRequestProperty("x-api-key", auth ?: ""); setRequestProperty("anthropic-version", "2023-06-01") } }
        currentCoroutineContext()[Job]?.invokeOnCompletion { c.disconnect() }; val text = StringBuilder()
        try {
            c.outputStream.use { it.write(body.toByteArray(Charsets.UTF_8)) }; val status = c.responseCode
            if (status !in 200..299) { val error = (c.errorStream ?: c.inputStream).bufferedReader().use { it.readText() }; throw IllegalStateException("HTTP $status: $error") }
            if (streaming) c.inputStream.bufferedReader().useLines { lines -> lines.forEach { line -> if (!line.startsWith("data:")) return@forEach; val data = line.removePrefix("data:").trim(); if (data.isBlank() || data == "[DONE]") return@forEach; val delta = runCatching { extract(JSONObject(data)) }.getOrDefault(""); if (delta.isNotEmpty()) { text.append(delta); onDelta(delta) } } }
            else { val json = JSONObject(c.inputStream.bufferedReader().use { it.readText() }); text.append(extract(json)); val usage = json.optJSONObject("usage") ?: json.optJSONObject("usageMetadata"); return ProviderResult(true, text.toString(), AiResponse(text.toString(), model, provider, usage?.optIntOrNull("prompt_tokens", "promptTokenCount"), usage?.optIntOrNull("completion_tokens", "candidatesTokenCount"))) }
            return ProviderResult(true, text.toString(), AiResponse(text.toString(), model, provider))
        } finally { c.disconnect() }
    }
    private fun defaultEndpoint(provider: String) = when { provider.equals("OpenAI", true) -> "https://api.openai.com/v1/chat/completions"; provider.equals("OpenRouter", true) -> "https://openrouter.ai/api/v1/chat/completions"; provider.equals("Groq", true) -> "https://api.groq.com/openai/v1/chat/completions"; provider.equals("Mistral", true) -> "https://api.mistral.ai/v1/chat/completions"; provider.equals("DeepSeek", true) -> "https://api.deepseek.com/chat/completions"; provider.equals("xAI", true) -> "https://api.x.ai/v1/chat/completions"; provider.equals("Together AI", true) -> "https://api.together.xyz/v1/chat/completions"; provider.equals("Fireworks AI", true) -> "https://api.fireworks.ai/inference/v1/chat/completions"; provider.equals("Cerebras", true) -> "https://api.cerebras.ai/v1/chat/completions"; provider.equals("Perplexity", true) -> "https://api.perplexity.ai/chat/completions"; provider.equals("DeepInfra", true) -> "https://api.deepinfra.com/v1/openai/chat/completions"; provider.equals("LiteLLM", true) -> "http://localhost:4000/v1/chat/completions"; provider.equals("Custom OpenAI-compatible", true) -> ""; else -> "" }
