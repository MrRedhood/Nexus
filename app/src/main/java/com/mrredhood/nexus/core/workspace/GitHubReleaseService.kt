...
class GitHubReleaseService {
    suspend fun list(repository: String, token: String, includeDrafts: Boolean = true): List<GitHubRelease> = withContext(Dispatchers.IO) {
        val root = JSONArray(requestText("GET", "/repos/$repository/releases?per_page=100", token, null))
    ...
    suspend fun create(repository: String, token: String, tagName: String, name: String, body: String, targetCommitish: String, draft: Boolean, prerelease: Boolean): GitHubRelease = withContext(Dispatchers.IO) {
        ...
        parseRelease(request("POST", "/repos/$repository/releases", token, JSONObject().apply {
            put("tag_name", tagName.trim()); put("name", name.trim()); put("body", body); put("target_commitish", targetCommitish.trim()); put("draft", draft); put("prerelease", prerelease)
        }))
    }
    ...
    suspend fun delete(repository: String, releaseId: Long, token: String) = withContext(Dispatchers.IO) {
        requestText("DELETE", "/repos/$repository/releases/$releaseId", token, null); Unit
    }
    ...
    suspend fun uploadAsset(repository: String, releaseId: Long, token: String, file: File, contentType: String): GitHubReleaseAsset = withContext(Dispatchers.IO) {
        ...
        val connection = connection("https://uploads.github.com/repos/$repository/releases/$releaseId/assets?name=$name", "POST", token).apply {
            setRequestProperty("Content-Type", contentType.ifBlank { "application/octet-stream" }); setFixedLengthStreamingMode(file.length()); doOutput = true
        }
        try {
            file.inputStream().use { input -> connection.outputStream.use { output -> input.copyTo(output) } }
            parseAsset(JSONObject(readResponse(connection)))
    ...
    private fun parseRelease(o: JSONObject): GitHubRelease {
        ...
        return GitHubRelease(
            id = o.getLong("id"), tagName = o.optString("tag_name"), targetCommitish = o.optString("target_commitish"), name = o.optString("name").ifBlank { o.optString("tag_name") }, body = o.optString("body"),
            draft = o.optBoolean("draft"), prerelease = o.optBoolean("prerelease"), createdAt = o.optString("created_at"), publishedAt = o.optString("published_at").ifBlank { null }, htmlUrl = o.optString("html_url"),
            assets = (0 until assetsJson.length()).map { parseAsset(assetsJson.getJSONObject(it)) }
        )
    }

    private fun parseAsset(o: JSONObject) = GitHubReleaseAsset(o.getLong("id"), o.optString("name"), o.optLong("size", 0L), o.optString("content_type", "application/octet-stream"), o.optString("browser_download_url"), o.optString("created_at"), o.optString("updated_at"))

    private fun request(method: String, path: String, token: String, body: JSONObject?): JSONObject = JSONObject(requestText(method, path, token, body).ifBlank { "{}" })
    ...
    private fun requestText(method: String, path: String, token: String, body: JSONObject?): String {
        ...
        return try { readResponse(connection) } finally { connection.disconnect() }
    }
    ...
    private fun connection(url: String, method: String, token: String): HttpURLConnection {
        ...
        return (URL(url).openConnection() as HttpURLConnection).apply {
            ...
            setRequestProperty("Accept", "application/vnd.github+json"); setRequestProperty("Authorization", "Bearer $token"); setRequestProperty("X-GitHub-Api-Version", "2022-11-28"); setRequestProperty("User-Agent", "Nexus-Android")
        }
    }
